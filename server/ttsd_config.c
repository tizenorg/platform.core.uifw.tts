...
void __ttsd_config_voice_changed_cb(const char* before_language, int before_type, const char* language, int type, bool auto_voice, void* user_data)
{
	...
	if (false == tts_config_check_default_voice_is_valid(language, type)) {
		SLOG(LOG_ERROR, tts_tag(), "Lang(%s) type(%d) is NOT valid : %s", language, type);
		return;
	}
			   ...
			   const char* func, int line, const char* message)
{
	...
	if (NULL != func) 	SLOG(LOG_ERROR, tts_tag(), "Function(%s) Line(%d)", func, line);
	if (NULL != message) 	SLOG(LOG_ERROR, tts_tag(), "Message(%s)", message);
	if (NULL != lang) 	SLOG(LOG_ERROR, tts_tag(), "Lang(%s), type(%d)", lang, vctype);
	if (NULL != text) 	SLOG(LOG_ERROR, tts_tag(), "Text(%s)", text);
